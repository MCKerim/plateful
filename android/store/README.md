...
`generate-splash-screens.mjs` regenerates the 26 `drawable*/splash.png` files
from the same master:

```sh
node android/store/generate-splash-screens.mjs
```

**These only affect Android 11 and below.** From Android 12 the system draws the
splash itself from the adaptive launcher icon and ignores the
`android:background="@drawable/splash"` in `AppTheme.NoActionBarLaunch` — no
`windowSplashScreenAnimatedIcon` / `windowSplashScreenBackground` is set
anywhere, so nothing overrides that default. On a modern device the new mascot
appeared the moment the launcher icon changed. They are still generated because
`minSdkVersion` is 24; raising it to 31 would let all 26 be deleted.

Two deliberate differences from the set they replace, which was generated by
tooling rather than designed:

- Backgrounds are the app's own page colours (`#faf9f5` / `#0a0905`) instead of
  `#FFFFFF` / `#111111`, so the handoff into the web view does not flash.
- The mascot is sized against the **short** side. The old art was sized against
  width, so it rendered half again as large in landscape as in portrait.
