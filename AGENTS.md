...
This file provides guidance to coding agents (Claude Code, Codex and cloud sessions) when working with code in this repository. `CLAUDE.md` only imports this file.
...
A `meal_planning` row is one placement (a day, or the pool with `planned_date` null) carrying exactly one kind of content: a recipe (`recipe_id`, `eaten` true/false) or a note (`note_id` → `planner_notes`, `eaten` NULL) for a day the household won't cook ("Eating out"). `MealPlannerItem` is the discriminated union `PlannedRecipe | PlannedNote` (`kind`); a note's copies made through "Edit plan" share `noteId` and therefore one text. Writes: `create_planner_note` RPC (note + first placement, idempotent per client-minted id), `PATCH planner_notes` for the text, `apply_planner_changes` RPC for the weekly dialog's delta (recipes AND notes, one transaction), plain `PATCH`/`DELETE meal_planning` for moves and removal; the last placement's delete removes the note row on the server. Notes have no cooked state, no rating and no `plan_meals` mission count. The note dialog's suggestion chips lead with the household's own texts (`planner_note_suggestions` RPC: placements counted, case-insensitive, up to 30 characters, count then recency) and fill up with the four built-ins through `mergeNoteSuggestions` (six at most), the same rule as the native app. **`planned_date` is a Postgres `date`: send a local `yyyy-MM-dd` (`toPlannedDateString`), never `toISOString()`**, which lands on the previous day between local midnight and 02:00. Server contract and the iOS side: `docs/knowledge/planner-notes.md` in `~/programming/ios-native/plateful`; the plan this was built from: `docs/planner-notes-plan.md`.

### Week start

The planner's first weekday is the account's own, in `users.week_start` (0 = Sunday .. 6 = Saturday, the server's numbering shared with the plan reminders' `day_of_week`). The first client that sees an account without one seeds it from the device through the `seed_week_start` RPC (it writes only a still-null row and returns what the account holds, so two devices cannot overwrite each other); here the device's week is the browser locale's week info (`deviceWeekStart` in `src/lib/weekStart.ts`, Monday where the browser cannot say). After that only the picker in Settings > General changes it, and later changes of the device's own setting are ignored on purpose, so the server (the weekly reminder, and week-planning features to come) never has to guess. Every week this app builds goes through `getWeekdays(date, weekStart)` and `isSameWeek(..., { weekStartsOn })` with the value from `useWeekStart()` (Redux user state, seeded in `useUserData`). Contract and the iOS side: `docs/knowledge/week-start.md` in `~/programming/ios-native/plateful`.
...
A share is a JSON **snapshot** of a recipe in `shared_recipes` (server-generated token + jsonb), created through the `create-recipe-share` Edge Function (JWT + household check, cover copied with the service role) and resolved through the `resolve_recipe_share` RPC. The snapshot keys are a **cross-platform contract with the native iOS app** (snake_case; every key added after the first shares is optional): the recipe fields, `ingredients[]`, `instruction_steps[]` with a per-step `annotation` (the stored cooking-mode annotation, ingredient mentions as `ingredient_index` into `ingredients` instead of a row id), `nutrition` (seven columns, `null` = unset) and `nutrition_auto`. Create lives in `src/hooks/recipe/useCreateRecipeShare.ts`, import in `src/api/recipeShare.api.ts` (mints the ingredient row ids client-side so the annotations can point at them; asks for a nutrition refresh only when the snapshot has none), the rewrites in `src/lib/transformers/recipeShare.transformer.ts`. This app renders no cooking-mode chips, but it must keep carrying the annotations so iOS members of the importing household get them. **Link previews:** `vercel.json` rewrites `/share/:token` to the edge function `api/share.ts`, which resolves the token through the same RPC and serves `/index.html` with the recipe's Open Graph tags injected (`src/lib/sharePreview.ts`, unit-tested), so WhatsApp and iMessage show a card with name, description and cover; a miss or failure serves the plain shell. Files in `api/` deploy as Vercel functions, so never put tests there (the handler test lives in `src/lib/sharePreview.handler.test.ts`). Canonical contract notes: the iOS repo's `docs/knowledge/recipe-sharing.md`.
