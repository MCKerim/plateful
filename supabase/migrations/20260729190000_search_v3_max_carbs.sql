-- Search v3 (NL layer): add the missing derived-fact filter — "low carb" must
-- map to a real constraint on carbs_g, like calories and protein already do.
-- New parameter = new signature: drop + recreate + re-grant.

drop function if exists public.search_recipes(text, uuid, integer, integer, text[], text[], text[], integer, integer, integer, text);
...
create function public.search_recipes(
  ...
  p_max_carbs_g         integer  default null
)
returns table (
  id                 uuid,
...
as $$
  ...
  where rw.status = 'ready'
    ...
    and (p_max_carbs_g is null or r.carbs_g <= p_max_carbs_g)
    and (q.txt is null
...
revoke execute on function public.search_recipes(text, uuid, integer, integer, text[], text[], text[], integer, integer, integer, text, integer) from public, anon;
grant execute on function public.search_recipes(text, uuid, integer, integer, text[], text[], text[], integer, integer, integer, text, integer) to authenticated, service_role;
