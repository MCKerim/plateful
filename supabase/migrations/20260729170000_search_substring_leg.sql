-- Live-typing search (decided 2026-07-29): the app now runs the server search
-- debounced on every pause in typing instead of only on the return key. For
-- that to feel right, the server's results must be a SUPERSET of the app's
-- instant local filter (name-contains) — otherwise a mid-word query like
-- "Lasag" can show local matches that then vanish when the server responds
-- (full-text matches whole words only, and trigram similarity is unreliable
-- under ~4 characters). A plain name-substring leg closes that gap; matches it
-- contributes rank below full-text hits. LIKE wildcards in user text only ever
-- widen the match, so they are left unescaped. Same signature as before, so
  ...
  select rw.id, rw.name, rw.description, rw.created_at, rw.status,
         ...
         (case
            when q.txt is null then 0
               ...
               -- the local filter's semantics, mirrored (see header comment)
